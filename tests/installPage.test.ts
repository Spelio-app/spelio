...
assert(installPageSource.includes('badgeAlt={t(\'install.appStoreBadgeAlt\')}'), 'App Store badge link should have translated accessible text.');
assert(installPageSource.includes('badgeAlt={t(\'install.googlePlayBadgeAlt\')}'), 'Google Play badge link should have translated accessible text.');
assert(installPageSource.includes('<img src={badgeSrc} alt={badgeAlt} />'), 'Store badge images should supply their accessible names to the links.');
assert(!installPageSource.includes("t('install.appStoreTitle')"), 'App Store option should not repeat the store name as a heading.');
assert(!installPageSource.includes("t('install.googlePlayTitle')"), 'Google Play option should not repeat the store name as a heading.');
assert(!installPageSource.includes("t('install.appStoreBody')"), 'App Store option should not repeat the badge action as body copy.');
assert(!installPageSource.includes("t('install.googlePlayBody')"), 'Google Play option should not repeat the badge action as body copy.');
...
assert(!stylesSource.includes('.install-option{'), 'Store options should not use the old card container.');
assert(!stylesSource.includes('.install-option-primary'), 'Store options should not use a highlighted card treatment.');
assert(stylesSource.includes('.install-badge-link-app-store img{\n  width:180px;'), 'App Store badge should be noticeably larger than its previous 148px width.');
assert(stylesSource.includes('.install-badge-link-google-play img{\n  width:190px;'), 'Google Play badge should be noticeably larger than its previous 148px width.');
...
assert(!enSource.includes('Download Spelio from Google Play.'), 'English Google Play option should rely on its official badge.');
assert(!enSource.includes('Download Spelio from the App Store.'), 'English App Store option should rely on its official badge.');
assert(enSource.includes("appStoreEyebrow: 'iPhone and iPad'"), 'English App Store platform label should remain concise.');
assert(enSource.includes("googlePlayEyebrow: 'Android'"), 'English Google Play platform label should remain concise.');
...
assert(!cySource.includes('Lawrlwythwch Spelio o Google Play.'), 'Welsh Google Play option should rely on its official badge.');
assert(!cySource.includes('Lawrlwythwch Spelio o’r App Store.'), 'Welsh App Store option should rely on its official badge.');
assert(cySource.includes("appStoreEyebrow: 'iPhone ac iPad'"), 'Welsh App Store platform label should remain translated.');
assert(cySource.includes("googlePlayEyebrow: 'Android'"), 'Welsh Google Play platform label should remain concise.');
