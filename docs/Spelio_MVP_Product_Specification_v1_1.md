...
4. Else recommend the next unfinished list by explicit collection order, then word-list order.
5. Else recommend weakest incomplete list.
...
Catalogue/display order is controlled by word-list order within the relevant collection/category display. Curated progression is controlled by `nextListId`. When following `nextListId`, Continue learning should skip active lists that already have the same full-completion state used for the Word Lists completion tick, but must not skip merely attempted, amber/in-progress, progression-complete, or unresolved-difficulty lists. If `nextListId` is empty, loops, broken, or otherwise unusable, recommendations may fall back to collection/list order. Do not add a separate progression order number unless a future content need proves it necessary.
...
- The native input's resulting `input` event is authoritative for printable characters, including while composition remains active; printable `keydown`, `beforeinput`, and `compositionend` events must not also update spelling. Some mobile keyboards keep one composition active across multiple ordinary keystrokes and Spelio word transitions, so cumulative native buffers must be acknowledged and processed incrementally from only their newly appended logical characters. `compositionend` is not required for ordinary character commitment and must never replay acknowledged buffer content. A delayed `compositionend` fallback may commit only a genuinely unacknowledged suffix when the browser omits usable `input` events. Each fallback is associated with its composition cycle, word, and answer position, and the fallback plus any later matching native input must be idempotent. A late duplicate must never validate against the next character. One physical or touch action must never validate or advance more than once.
